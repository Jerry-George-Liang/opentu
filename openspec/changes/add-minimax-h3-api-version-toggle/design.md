...
- H3 参考视频使用 content[].video_url + role=reference_video。公网 HTTP(S) 地址尝试读取 MP4/MOV 并转为 Data URL，读取失败或类型不匹配时回退原 URL；本地 `/asset-library/` 虚拟地址仅在请求时从统一缓存读取 MP4/MOV Blob 并转换为 Data URL，不把 Base64 持久化进工作流。
- 官方请求体上限为 64 MiB，而 Base64 会使文件体积增加约三分之一。本地内联视频合计限制为 47 MiB，并在序列化后再次校验完整 JSON 不超过 64 MiB；更大视频使用公网 URL。本轮不新增后端暂存上传接口。
...
参考视频仅接受 MP4/MOV、本地素材或公网 HTTP(S) 地址，数量最多 3 个。本地缓存缺失、格式不支持、本地视频合计超过 47 MiB，或最终 JSON 超过 64 MiB 时，均在发送供应商请求前终止并显示可操作错误。其他不支持视频引用的模型继续走既有能力校验。
...
- 本地视频 Data URL 会增加浏览器内存占用；47 MiB 总量限制是无后端上传接口时的保守策略，公网 URL 是大文件路径。
- 不迁移存储，不删除已有任务。回滚功能代码即可撤回新入口；保留已有任务数据。
- 页面交互、真实计费接口与语言遵循情况须人工验收，mock 测试不能代替上游验收。

## Verification

参数和路由测试验证固定 V2；工作流测试覆盖增强成功、关闭、失败、超时、语言指令、本地视频缓存转换和大小边界；视频服务测试验证最终发送体；重制测试覆盖资格与源供应商；任务队列测试覆盖 remoteId 进度和终态回写。实际结果见 qa/2026-09-15-MiniMax-H3-V2工作流-验证记录.md。

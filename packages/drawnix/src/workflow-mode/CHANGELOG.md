...
+ [新增] 视频工作台支持将选中成功记录批量保存到我的资产，保存独立视频副本；历史视频图标支持拖入参考区，按当前模型检查参考视频能力、数量与地址格式。

+ [新增] 生图工作台支持将选中历史记录的全部结果保存到我的资产，保留原记录提示词；历史缩略图可拖到参考图区，使用原图创建独立副本。

+ [调整] 画布默认生图张数改为 1，未保存该偏好时也使用 1，单个节点仍可独立设置数量。

+ [调整] 工作流初始配置与新增 OpenAI 协议渠道默认使用 `https://api.tu-zi.com`，请求继续自动补齐 `/v1`；已有保存地址保持原值。

+ [修复] 本地嵌入工作流读取兔子渠道模型时复用宿主同源代理，避免上游错误响应缺少 CORS 头导致只显示“请求失败”；拉取失败保留具体错误，原生渠道读取模型不再要求子页面持有密钥。

+ [调整] 嵌入模式的六个工作流导航入口移至顶部返回栏，画布编辑页也可直接切换；当前页面同步高亮，窄屏导航横向滚动，移除重复导航栏。

+ [优化] 模型选择器将模型名称与渠道分成两行，长名称完整换行；下拉列表按视口限制宽高，强化选中态，触发控件仅显示模型名并保留完整悬停提示。同步收紧原生参数面板标题与字段间距。

+ [新增] 工作流按 OpenTu 完整模型目录及实际渠道绑定同步参数、默认值、条件选项和参考素材约束，图片、视频、文字与音频节点及工作台使用同一参数契约。
+ [修复] 原生模型参数按渠道、模型和能力独立保存，恢复与重试保留原值；提交前验证类型、枚举、范围与素材，MiniMax-H3 等模型不再被通用像素/清晰度设置覆盖。
+ [修复] 补齐文字系统提示词和对话上下文、Suno 专属字段与歌词/多段音频回填、视频参考素材，以及 Midjourney 专属参数的实际消费。
+ [调整] 逐模型记录参数消费者和宿主不可用原因；未知契约、缺失适配器或当前绑定不支持的字段明确提示。模型出现在目录中不代表供应商已开通。
+ [修复] 四类节点参数弹层统一按视口定位和滚动，窄屏不再溢出；同名多能力模型保持调用脚本与参数隔离。

+ [修复] 节点模型与配置页默认模型保持一致，未配置的能力显示“选择模型”并阻止误用原版默认模型，手动选择仍按节点保存。

+ [修复] 修正嵌入工作流模型图标的资源基路径，避免模型下拉框显示破图。

+ [调整] 工作流同步默认渠道全部能力及各渠道手工绑定模型，重新进入时补充最新模型目录，保留现有选择和脚本。

+ [修复] 补齐迁移后 default 分组（legacy-default）的内置模型列表，修复偏好设置只显示一个默认候选的问题。

+ [调整] 工作流模型下拉扩展为默认渠道的 OpenTu 内置模型和各启用渠道已发现的模型，保留原默认选择。

+ [修复] 工作流模型选择接入 OpenTu 原生渠道和生成适配器，补齐媒体模型并保留自定义配置，生成结果留在工作流。

+ [新增] 工作流使用 /workflow 独立入口，支持直接访问、刷新及浏览器前进后退，返回时恢复普通画布地址。

+ [新增] 工作流首次初始化可带入 OpenTu 兼容渠道和默认模型，保留已有自定义配置并提示未支持的协议。

+ [调整] 工作流保留原版黑白主题样式，默认浅色并支持切换深色，隐藏导航品牌、版本号和 GitHub 入口。

+ [新增] OpenTu 工作流入口以独立构建页面嵌入无限画布，支持返回普通画布并保留页面状态。

...
export type AudioGenerationResult = { kind: "audio"; clips: Array<Blob | string> } | { kind: "lyrics"; text: string };
export type StoredAudioGenerationResult = { kind: "audio"; clips: UploadedFile[] } | { kind: "lyrics"; text: string };
...
export async function requestAudioGeneration(config: AiConfig, prompt: string, options?: RequestOptions): Promise<AudioGenerationResult> {
    ...
    if (script) {
        ...
        try {
            ...
            return { kind: "audio", clips: [await audioPluginBlob(result, format)] };
        } catch (error) {
    ...
    try {
        ...
        return { kind: "audio", clips: [response.data.type.startsWith("audio/") ? response.data : new Blob([response.data], { type: audioMimeType(format) })] };
    } catch (error) {
        throw new Error(readAxiosError(error, apiText("audioGenerationFailed")));
    }
}

export async function storeGeneratedAudioResult(result: AudioGenerationResult, format = "mp3", options?: RequestOptions): Promise<StoredAudioGenerationResult> {
    if (result.kind === "lyrics") return result;
    const clips = await Promise.all(result.clips.map(async (clip) => {
        if (options?.signal?.aborted) throw new DOMException("Aborted", "AbortError");
        if (typeof clip !== "string") return storeGeneratedAudio(clip, format);
        let response: Response;
        try {
            response = await fetch(withLocalProxy(clip), { signal: options?.signal });
        } catch (error) {
            if (options?.signal?.aborted) throw new DOMException("Aborted", "AbortError");
            // Remote provider assets may disallow browser downloads but remain playable.
            if (/^https?:/i.test(clip)) return { url: clip, storageKey: "", bytes: 0, mimeType: audioMimeType(format) };
            throw error;
        }

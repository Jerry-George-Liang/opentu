...
export async function fetchImageModels(config: Pick<AiConfig, "baseUrl" | "apiKey" | "apiFormat">) {
    try {
        ...
        if (!Array.isArray(response.data?.data)) throw new Error("模型列表响应格式不正确，请检查接口地址和协议。");
        ...
        if (axios.isAxiosError(error) && !error.response && error.code === "ERR_NETWORK") {
            throw new Error("无法连接模型列表接口：请检查接口地址、网络及本地代理是否启动；浏览器跨域限制也可能阻止请求。");
        }
...
export async function fetchChannelModels(channel: ModelChannel) {
    if (channel.opentuProfileId !== undefined) return channel.models.map((model) => model.name);
    return fetchImageModels({ baseUrl: channel.baseUrl, apiKey: channel.apiKey, apiFormat: channel.apiFormat });
}

async function requestNativeImages(config: AiConfig, prompt: string, references: ReferenceImage[], options?: RequestOptions) {
    const model = config.model || config.imageModel;
    const result = await requestNative(config, model, {

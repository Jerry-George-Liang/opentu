...
const OPENAI_BASE_URL = "https://api.tu-zi.com";
...
type ConfigStore = {
    opentuDefaultsInitialized: boolean;
    config: AiConfig;
...
function findChannelModel(config: AiConfig, value: string, capability?: ModelCapability): { channel: ModelChannel; model: ChannelModel } | null {
    const decoded = decodeChannelModel(value);
    const name = decoded?.model || value;
    const matches = (item: ChannelModel) => item.name === name && (!capability || item.capability === capability);
    const channel = decoded ? config.channels.find((item) => item.id === decoded.channelId) : config.channels.find((item) => item.models.some(matches));
    const model = channel?.models.find(matches);
...
export function modelMatchesCapability(config: AiConfig, value: string, capability?: ModelCapability) {
    ...
    const decoded = decodeChannelModel(value);
    return config.channels.some((channel) => (!decoded || channel.id === decoded.channelId) && channel.models.some((model) => model.name === (decoded?.model || value) && model.capability === capability));
}

export function resolveModelForCapability(config: AiConfig, currentModel: string | undefined, capability: ModelCapability) {
    const defaultModel = capability === "image" ? config.imageModel : capability === "video" ? config.videoModel : capability === "audio" ? config.audioModel : config.textModel;
    if (currentModel && modelMatchesCapability(config, currentModel, capability)) return currentModel;
    if (currentModel && !findChannelModel(config, currentModel)) return "";
...
export function resolveModelScript(config: AiConfig, value: string, capability?: ModelCapability) {
    return findChannelModel(config, value, capability)?.model.script?.trim() || "";
}
...
export const useConfigStore = create<ConfigStore>()(
    persist(
        (set, get) => ({
            opentuDefaultsInitialized: false,
        ...
        }),
        {
            ...
            partialize: (state) => ({ config: state.config, webdav: state.webdav, opentuDefaultsInitialized: state.opentuDefaultsInitialized }),
...
export function normalizeChannelModels(models: Array<string | ChannelModel> | undefined): ChannelModel[] {
    ...
    for (const item of models || []) {
        ...
        result.push({ ...(typeof item === "string" ? {} : item), name, capability, script });
    }
    return result;
}
...
export function resolveModelChannel(config: AiConfig, value: string, capability?: ModelCapability) {
    const decoded = decodeChannelModel(value);
    const model = decoded?.model || value;
    const matched = decoded ? config.channels.find((channel) => channel.id === decoded.channelId) : config.channels.find((channel) => channel.models.some((item) => item.name === model && (!capability || item.capability === capability)));

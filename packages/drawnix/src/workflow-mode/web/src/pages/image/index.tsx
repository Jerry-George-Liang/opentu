...
type GenerationLogConfig = Pick<AiConfig, "model" | "imageModel" | "quality" | "size" | "count" | "background" | "nativeParams" | "systemPrompt">;

type UpdateAiConfig = <K extends keyof AiConfig>(key: K, value: AiConfig[K]) => void;

const LOG_STORE_KEY = "infinite-canvas:image_generation_logs";
const HISTORY_IMAGE_DRAG_TYPE = "application/x-opentu-history-image";
...
export default function ImagePage() {
    ...
    const generate = async () => {
        ...
        const generationCount = snapshot.generationCount;
    ...
    const saveSelectedToAssets = async () => {
        ...
        let saved = 0;
        let failed = 0;
        for (const log of selected) for (const [index, image] of log.images.entries()) {
            try {
                const stored = await uploadImage(image.storageKey ? await resolveImageUrl(image.storageKey) || image.dataUrl : image.dataUrl);
                addAsset({ kind: "image", title: `${log.title} ${index + 1}`, coverUrl: stored.url, tags: [], source: t("imageWorkbench.source"),
                    data: { dataUrl: stored.url, storageKey: stored.storageKey, width: stored.width, height: stored.height, bytes: stored.bytes, mimeType: stored.mimeType },
                    metadata: { source: "image-page", prompt: log.prompt, model: log.model } });
                saved += 1;
            } catch { failed += 1; }
        }
        setSavingLogAssets(false);
        if (saved) message.success(`已保存 ${saved} 张图片到我的资产`);
        if (failed) message.error(`${failed} 张图片保存失败，请重试`);
    };

    const addHistoryReference = async (payload: string) => {
        try {
            const { logId, imageId } = JSON.parse(payload);
    ...
    const previewGenerationLog = async (log: GenerationLog) => {
        if (previewLog?.id === log.id) {
            ...
            setElapsedMs(0);
            setStartedAt(0);
            setSelectedLogIds((ids) => ids.filter((id) => id !== log.id));
            return;
        }
        setPreviewLog(log);
        setSelectedLogIds((ids) => ids.includes(log.id) ? ids : [...ids, log.id]);
        setLogsOpen(false);
    ...
    const buildRequestSnapshot = () => {
        ...
        try {
            const generationCount = nativeImageCount(selectedConfig) ?? Math.max(1, Math.min(10, Number(effectiveConfig.count) || 1));
            return { text, config: { ...selectedConfig, imageModel: model }, generationCount, references: [...references] };
        } catch (error) {
            message.error(error instanceof Error ? error.message : t("imageWorkbench.invalidParams"));
            return null;
        }
    };
            ...
            <main className="grid min-h-0 flex-1 grid-cols-1 gap-3 overflow-y-auto p-3 lg:grid-cols-[300px_minmax(0,1fr)] lg:overflow-hidden xl:grid-cols-[320px_minmax(0,1fr)]">
                <aside className="thin-scrollbar hidden min-h-0 overflow-y-auto rounded-lg border border-stone-200 bg-card p-4 shadow-sm dark:border-stone-800 lg:block">
                    <LogPanel
                        ...
                        onSaveSelected={() => void saveSelectedToAssets()}
                        savingAssets={savingLogAssets}
                        ...
                        <div className="mt-6 space-y-5">
                            ...
                            <div className="min-w-0">
                                ...
                                <div
                                    ...
                                    onDragEnter={(event) => {
                                        ...
                                        if (event.dataTransfer.types.includes("Files") || event.dataTransfer.types.includes(HISTORY_IMAGE_DRAG_TYPE)) setIsReferenceDragActive(true);
                                    }}
                                    onDragOver={(event) => {
                                        event.preventDefault();
                                    ...
                                    onDrop={(event) => {
                                        ...
                                        const historyImage = event.dataTransfer.getData(HISTORY_IMAGE_DRAG_TYPE);
                                        if (historyImage) void addHistoryReference(historyImage);
                            ...
                            <div className="flex items-center justify-between rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-sm dark:border-stone-800 dark:bg-stone-900 sm:hidden">
                                <span className="truncate text-stone-500 dark:text-stone-400">
                                    {modelOptionLabel(effectiveConfig, model)} · {nativeParameterSummary(selectedConfig, "image") ?? `${effectiveConfig.size} · ${effectiveConfig.quality}`}
                                </span>
    ...
    return (
        <>
            ...
            <div className="mb-4 flex flex-wrap gap-2">
                ...
                <Tooltip title={t("common.addToAssets")}>
                    <Button size="small" aria-label={t("common.addToAssets")} icon={<FolderPlus className="size-3.5" />} loading={savingAssets} disabled={!logs.some((log) => selectedLogIds.includes(log.id) && log.images.length)} onClick={onSaveSelected} />
                </Tooltip>
            </div>
                ...
                {logs.map((log) => (
                    <LogCard
                        ...
                        onSelectedChange={(checked) => {
                            ...
                            if (checked !== (activeLogId === log.id)) onPreviewLog(log);
                        }}
                        ...
                        {thumbnails.length ? (
                            <div className="mt-2 flex gap-1 overflow-hidden">
                                {thumbnails.map((image) => (
                                    <img key={image.id} src={previewUrlFor(image.storageKey) || image.dataUrl} alt="" draggable title="拖到参考图区" onDragStart={(event) => {
                                        event.stopPropagation();
                                        event.dataTransfer.effectAllowed = "copy";
                                        event.dataTransfer.setData(HISTORY_IMAGE_DRAG_TYPE, JSON.stringify({ logId: log.id, imageId: image.id }));
                                    }} className="size-8 shrink-0 cursor-grab rounded-md object-cover active:cursor-grabbing" />
                                ))}
                            </div>
                        ) : null}
                    </div>
...
export function normalizeLogConfig(log: Partial<GenerationLog>): GenerationLogConfig {
    return {
        ...
        systemPrompt: log.config?.systemPrompt || "",
    };
}

...
type GenerationLogConfig = Pick<AiConfig, "model" | "videoModel" | "size" | "vquality" | "videoSeconds" | "videoGenerateAudio" | "videoWatermark" | "videoMode" | "nativeParams">;

type UpdateAiConfig = <K extends keyof AiConfig>(key: K, value: AiConfig[K]) => void;

const LOG_STORE_KEY = "infinite-canvas:video_generation_logs";
const HISTORY_VIDEO_DRAG_TYPE = "application/x-opentu-history-video";
...
export default function VideoPage() {
    ...
    const [referenceLinks, setReferenceLinks] = useState({ videos: "", audios: "" });
    ...
    const [savingLogAssets, setSavingLogAssets] = useState(false);
    ...
    const addReferences = async (files?: FileList | null) => {
        ...
        if (nativeEntry && selectedFiles.filter((file) => file.type.startsWith("image/")).length + references.length > referenceLimit) {
            message.error("参考图片数量超出当前模型限制。");
            return;
        }
    ...
    const addReferenceMedia = async (kind: "videos" | "audios", files: FileList | null) => {
        const input = nativeEntry?.referenceInputs?.[kind];
        if (!input || (input.formats && !input.formats.includes("data"))) return;
        const selected = Array.from(files || []).filter((file) => file.type.startsWith(kind === "videos" ? "video/" : "audio/"));
        const current = kind === "videos" ? referenceVideos : referenceAudios;
        if (input.maxCount !== undefined && current.length + selected.length > input.maxCount) {
            message.error("参考素材数量超出当前模型限制。");
            return;
        }
        try {
            const added = await Promise.all(selected.map(async (file) => {
                const stored = await uploadMediaFile(file, kind === "videos" ? "video" : "audio");
                return { id: nanoid(), name: file.name, type: stored.mimeType, url: stored.url, storageKey: stored.storageKey, durationMs: stored.durationMs };
            }));
            if (kind === "videos") setReferenceVideos((value) => [...value, ...added]);
            else setReferenceAudios((value) => [...value, ...added]);
        } catch (error) { message.error(error instanceof Error ? error.message : t("workbench.generationFailed")); }
    };

    const addReferenceLink = (kind: "videos" | "audios") => {
        const url = referenceLinks[kind].trim();
        if (!url) return;
        const nextVideos = [...referenceVideos.map((item) => item.url), ...(kind === "videos" ? [url] : [])];
        const nextAudios = [...referenceAudios.map((item) => item.url), ...(kind === "audios" ? [url] : [])];
        try {
            validateNativeReferences({ [kind]: nativeEntry?.referenceInputs?.[kind] }, { [kind]: kind === "videos" ? nextVideos : nextAudios });
            const reference = { id: nanoid(), name: url, url, type: kind === "videos" ? "video/mp4" : "audio/mpeg" };
            if (kind === "videos") setReferenceVideos((value) => [...value, reference]);
            else setReferenceAudios((value) => [...value, reference]);
            setReferenceLinks((value) => ({ ...value, [kind]: "" }));
        } catch (error) { message.error(error instanceof Error ? error.message : t("videoWorkbench.invalidParams")); }
    };

    const handleReferenceDragEnter = (event: DragEvent<HTMLDivElement>) => {
        event.preventDefault();
        dragDepthRef.current += 1;
        if (event.dataTransfer.types.includes("Files") || event.dataTransfer.types.includes(HISTORY_VIDEO_DRAG_TYPE)) setReferenceDragTarget(true);
    };
    ...
    const handleReferenceDrop = (event: DragEvent<HTMLDivElement>) => {
        ...
        const historyVideo = event.dataTransfer.getData(HISTORY_VIDEO_DRAG_TYPE);
        if (historyVideo) void addHistoryVideoReference(historyVideo);
        else void addReferences(event.dataTransfer.files);
    };
    ...
    const addReferencesFromClipboard = async () => {
        try {
            ...
            if (nativeEntry && blobs.length + references.length > referenceLimit) {
                message.error("参考图片数量超出当前模型限制。");
                return;
            }
            const nextReferences = await Promise.all(
    ...
    const buildRequestSnapshot = () => {
        ...
        try {
            if (nativeEntry) {
                getNativeParameterValues(selectedConfig, model, "video");
                validateNativeReferences(nativeEntry.referenceInputs || {}, { images: references.map((item) => item.dataUrl), videos: referenceVideos.map((item) => item.url), audios: referenceAudios.map((item) => item.url) });
            }
            ...
            message.error(error instanceof Error ? error.message : t("videoWorkbench.invalidParams"));
            return null;
        }
    };
    ...
    const saveVideoAsset = async (video: GeneratedVideo, title: string, sourcePrompt: string) => {
        const stored = await uploadMediaFile(await resolveMediaUrl(video.storageKey, video.url), "video");
        addAsset({
            kind: "video",
            title,
            coverUrl: "",
    ...
    const saveResultToAssets = async (video: GeneratedVideo) => {
        try { await saveVideoAsset(video, t("videoWorkbench.resultTitle"), prompt); message.success(t("common.addedToAssets")); }
        catch { message.error("视频保存失败，请重试"); }
    };

    const saveSelectedToAssets = async () => {
        if (savingLogAssets) return;
        setSavingLogAssets(true);
        let saved = 0;
        let failed = 0;
        for (const log of logs.filter((item) => selectedLogIds.includes(item.id) && item.status === "success" && item.video)) {
            try { await saveVideoAsset(log.video!, log.title, log.prompt); saved += 1; }
            catch { failed += 1; }
        }
        setSavingLogAssets(false);
        if (saved) message.success(`已保存 ${saved} 个视频到我的资产`);
        if (failed) message.error(`${failed} 个视频保存失败，请重试`);
    };

    const addHistoryVideoReference = async (payload: string) => {
        try {
            const log = logs.find((item) => item.id === payload);
            if (!log?.video) throw new Error("无法读取历史视频");
            const input = nativeEntry?.referenceInputs?.videos;
            if (nativeEntry && !input) throw new Error("当前模型不支持参考视频，请选择支持参考视频的模型。");
            const url = await resolveMediaUrl(log.video.storageKey, log.video.url);
            if (input?.formats && !input.formats.includes("data") && !/^https?:\/\//i.test(url)) throw new Error("当前模型只接受公开视频地址，不能使用本地历史视频。");
            if (input?.maxCount !== undefined && referenceVideos.length >= input.maxCount) throw new Error("参考视频数量超出当前模型限制。");
            const urlOnly = input?.formats && !input.formats.includes("data");
            const stored = urlOnly ? { url, mimeType: log.video.mimeType, storageKey: undefined, width: log.video.width, height: log.video.height } : await uploadMediaFile(url, "video");
            setReferenceVideos((value) => [...value, { id: nanoid(), name: log.title, type: stored.mimeType, url: stored.url, storageKey: stored.storageKey, width: stored.width, height: stored.height }]);
            message.success("已加入参考视频");
        } catch (error) { message.error(error instanceof Error ? error.message : "无法读取历史视频"); }
    };
    ...
    const insertPickedAsset = async (payload: InsertAssetPayload) => {
        if (payload.kind === "text") {
            ...
            if (references.length >= referenceLimit) { message.error("参考图片数量超出当前模型限制。"); return; }
            const stored = await uploadImage(payload.dataUrl);
            setReferences((value) => [...value, { id: nanoid(), name: payload.title, type: stored.mimeType, dataUrl: stored.url, storageKey: stored.storageKey }].slice(0, referenceLimit));
        } else if (payload.kind === "video") {
            const input = nativeEntry?.referenceInputs?.videos;
            if (!input || (input.maxCount !== undefined && referenceVideos.length >= input.maxCount)) { message.error("当前模型不支持添加此参考视频。"); return; }
            setReferenceVideos((value) => [...value, { id: nanoid(), name: payload.title, type: "video/mp4", url: payload.url, storageKey: payload.storageKey, width: payload.width, height: payload.height }]);
        }
        ...
        <div className="flex h-full flex-col overflow-hidden bg-stone-50 text-stone-900 dark:bg-stone-950 dark:text-stone-100">
            <main className="grid min-h-0 flex-1 grid-cols-1 gap-3 overflow-y-auto p-3 lg:grid-cols-[300px_minmax(0,1fr)] lg:overflow-hidden xl:grid-cols-[320px_minmax(0,1fr)]">
                <aside className="thin-scrollbar hidden min-h-0 overflow-y-auto rounded-lg border border-stone-200 bg-card p-4 shadow-sm dark:border-stone-800 lg:block">
                    <LogPanel logs={logs} selectedLogIds={selectedLogIds} activeLogId={previewLog?.id} onSelectedLogIdsChange={setSelectedLogIds} onCreateSession={createSession} onDeleteSelected={() => setDeleteConfirmOpen(true)} onPreviewLog={previewGenerationLog} onSaveSelected={() => void saveSelectedToAssets()} savingAssets={savingLogAssets} />
                </aside>
                        ...
                        <div className="mt-6 space-y-5">
                            ...
                            <div className="min-w-0">
                                <div className="mb-2 flex items-center justify-between gap-3">
                                    ...
                                    <div className="flex gap-2">
                                        <Button size="small" disabled={referenceLimit === 0} icon={<ClipboardPaste className="size-3.5" />} onClick={() => void addReferencesFromClipboard()}>
                                            {t("workbench.clipboard")}
                                        </Button>
                                        <Button size="small" disabled={referenceLimit === 0} icon={<Upload className="size-3.5" />} onClick={() => fileInputRef.current?.click()}>
                                            {t("workbench.upload")}
                                        </Button>
                                    </div>
                                </div>
                                ...
                                >
                                    ...
                                    {!references.length ? <div className="flex min-w-full items-center justify-center text-sm text-stone-500">{referenceDragTarget ? t("videoWorkbench.dropReferences") : nativeEntry ? "暂无参考图片" : t("videoWorkbench.noImages")}</div> : null}
                                </div>
                            </div>

                            {(["videos", "audios"] as const).map((kind) => {
                                const items = kind === "videos" ? referenceVideos : referenceAudios;
                                const input = nativeEntry?.referenceInputs?.[kind];
                                if (!input && !items.length) return null;
                                const label = kind === "videos" ? "参考视频" : "参考音频";
                                return <div key={kind} className="min-w-0 space-y-2" onDragOver={kind === "videos" ? (event) => { event.preventDefault(); event.dataTransfer.dropEffect = "copy"; } : undefined} onDrop={kind === "videos" ? handleReferenceDrop : undefined}>
                                    <div className="flex items-center justify-between gap-2">
                                        <span className="text-sm font-semibold">{label}</span>
                                        {input && (!input.formats || input.formats.includes("data")) ? <Button size="small" icon={<Upload className="size-3.5" />} onClick={() => (kind === "videos" ? videoInputRef : audioInputRef).current?.click()}>{t("workbench.upload")}</Button> : null}
                                    </div>
                                    {input ? <div className="flex min-w-0 gap-2">
                                        <Input aria-label={`${label}地址`} placeholder={input.formats?.includes("asset") ? "https:// 或 asset://" : "https://"} value={referenceLinks[kind]} onChange={(event) => setReferenceLinks((value) => ({ ...value, [kind]: event.target.value }))} onPressEnter={() => addReferenceLink(kind)} />
                                        <Button type="text" aria-label={`添加${label}`} icon={<Plus className="size-4" />} onClick={() => addReferenceLink(kind)} />
                                    </div> : null}
                                    {items.map((item, index) => <div key={item.id} className="flex min-w-0 items-center gap-2 text-sm">
                                        <span className="shrink-0">{index + 1}</span>
                                        <span className="min-w-0 flex-1 truncate" title={item.name}>{item.name}</span>
                                        <Button type="text" size="small" disabled={index === 0} aria-label={`前移${label}`} icon={<ArrowLeft className="size-3.5" />} onClick={() => kind === "videos" ? setReferenceVideos((value) => moveListItem(value, index, -1)) : setReferenceAudios((value) => moveListItem(value, index, -1))} />
                                        <Button type="text" size="small" disabled={index === items.length - 1} aria-label={`后移${label}`} icon={<ArrowRight className="size-3.5" />} onClick={() => kind === "videos" ? setReferenceVideos((value) => moveListItem(value, index, 1)) : setReferenceAudios((value) => moveListItem(value, index, 1))} />
                                        <Button type="text" size="small" aria-label={`移除${label}`} icon={<Trash2 className="size-3.5" />} onClick={() => kind === "videos" ? setReferenceVideos((value) => value.filter((entry) => entry.id !== item.id)) : setReferenceAudios((value) => value.filter((entry) => entry.id !== item.id))} />
                                    </div>)}
                                </div>;
                            })}
                            ...
                            <div className="flex items-center justify-between rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-sm dark:border-stone-800 dark:bg-stone-900 sm:hidden">
                                <span className="truncate text-stone-500 dark:text-stone-400">
                                    {modelOptionLabel(effectiveConfig, model)} · {nativeParameterSummary(selectedConfig, "video") ?? `${normalizeResolution(effectiveConfig.vquality)}p · ${videoSizeLabel(effectiveConfig.size)} · ${normalizeVideoSeconds(effectiveConfig.videoSeconds)}s · ${videoModeLabel(effectiveConfig.videoMode)}`}
                                </span>
            ...
            <input ref={videoInputRef} type="file" accept="video/*" multiple className="hidden" onChange={(event) => { void addReferenceMedia("videos", event.target.files); event.target.value = ""; }} />
            <input ref={audioInputRef} type="file" accept="audio/*" multiple className="hidden" onChange={(event) => { void addReferenceMedia("audios", event.target.files); event.target.value = ""; }} />
        ...
        <>
            ...
            <div className="mb-4 flex flex-wrap gap-2">
                ...
                <Tooltip title={t("common.addToAssets")}>
                    <Button size="small" aria-label={t("common.addToAssets")} icon={<FolderPlus className="size-3.5" />} loading={savingAssets} disabled={!logs.some((log) => selectedLogIds.includes(log.id) && log.status === "success" && log.video)} onClick={onSaveSelected} />
                </Tooltip>
            </div>
...
function normalizeLogConfig(log: Partial<GenerationLog>): GenerationLogConfig {
    return {
        ...
        nativeParams: log.config?.nativeParams,
    };
}

export function buildLog({ prompt, model, config, references, referenceVideos, referenceAudios, durationMs, status, task, video, error }: { prompt: string; model: string; config: AiConfig; references: ReferenceImage[]; referenceVideos?: ReferenceVideo[]; referenceAudios?: ReferenceAudio[]; durationMs: number; status: GenerationLog["status"]; task?: VideoGenerationTask; video?: GeneratedVideo; error?: string }): GenerationLog {
    const logConfig = {
        model: config.model,
        ...
        nativeParams: config.nativeParams,
    };
    return {
        id: nanoid(),
        createdAt: Date.now(),
        ...
        video,
        error,
        parameterSummary: nativeParameterSummary(config, "video"),
    };
}

export function buildVideoConfig(config: AiConfig, model: string): AiConfig {
    if (nativeModel(config, "video", model)) return { ...config, model, videoModel: model };
    return {
        ...config,
        model,
        videoModel: model,

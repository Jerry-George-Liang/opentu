...
import { nativeImageCount, singleImageConfig } from "@/integration/native-parameters";
import { requestAudioGeneration, storeGeneratedAudioResult } from "@/services/api/audio";
import { audioResultNodes } from "@/lib/canvas/canvas-audio-results";
...
function InfiniteCanvasPage() {
    ...
    const completeAudioNodeTask = useCallback(async (nodeId: string, originNodeId: string, config: Parameters<typeof buildGenerationConfig>[0], prompt: string, signal: AbortSignal) => {
        ...
        if (!target) return;
        const generated = audioResultNodes(target, result, config, prompt);
    ...
    const createConnectedNode = useCallback(
        (type: CanvasNodeType.Image | CanvasNodeType.Text | CanvasNodeType.Config | CanvasNodeType.Video | CanvasNodeType.Audio, pending: PendingConnectionCreate) => {
            const metadata = type === CanvasNodeType.Config ? { size: effectiveConfig.size, count: getGenerationCount(effectiveConfig.canvasImageCount || effectiveConfig.count) } : undefined;
    ...
    const deleteNodes = useCallback(
        (ids: Set<string>) => {
            ...
            generationRequestsRef.current.forEach((request) => {
                if (allIds.has(request.targetNodeId) || allIds.has(request.originNodeId)) runningIds.add(request.runningNodeId);
            });
        ...
        (node: CanvasNodeData) => {
            ...
            const configNode = {
                ...createCanvasNode(
                    ...
                    {
                        generationMode: "text",
                        model: buildGenerationConfig(effectiveConfig, undefined, "text").model,
                        count: 1,
    ...
    const maskEditImageNode = useCallback(
        async (node: CanvasNodeData, payload: CanvasImageMaskEditPayload) => {
            if (!node.metadata?.content) return;
            const generationConfig = singleImageConfig({ ...buildGenerationConfig(effectiveConfig, node, "image"), size: node.metadata?.size || "auto" });
    ...
    const generateAngleNode = useCallback(
        async (node: CanvasNodeData, params: CanvasImageAngleParams) => {
            if (!node.metadata?.content) return;
            const generationConfig = singleImageConfig(buildGenerationConfig(effectiveConfig, node, "image"));
            if (!isAiConfigReady(generationConfig, generationConfig.model)) {
                openConfigDialog(true);
                return;
            }
    ...
    const handleGenerateNode = useCallback(
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            try {
                if (mode === "image") {
                    const count = nativeImageCount(generationConfig) ?? getGenerationCount(generationConfig.count);
                ...
                if (mode === "video") {
                    ...
                    const videoNode: CanvasNodeData = {
                        ...
                        metadata: {
                            ...
                            status: NODE_STATUS_LOADING,
                            model: generationConfig.model,
                            nativeParams: generationConfig.nativeParams,
                            size: generationConfig.size,
                            seconds: generationConfig.videoSeconds,
                            vquality: generationConfig.vquality,
                            generateAudio: generationConfig.videoGenerateAudio,
                ...
                if (mode === "audio") {
                    ...
                    try {
                        await completeAudioNodeTask(audioId, nodeId, generationConfig, effectivePrompt, controller.signal);
                    } finally {
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData, imageId?: string) => {
            ...
            const isAudioResult = node.type === CanvasNodeType.Audio || node.metadata?.generationMode === "audio";
            ...
            const generationConfig =
                hasSavedImageMetadata && savedImageMetadata
                    ? {
                          ...effectiveConfig,
                          model: buildGenerationConfig(effectiveConfig, { ...sourceNode, metadata: { ...sourceNode.metadata, model: savedImageMetadata.model } }, "image").model,
                          nativeParams: savedImageMetadata.nativeParams ?? sourceNode.metadata?.nativeParams ?? effectiveConfig.nativeParams,
                          quality: savedImageMetadata.quality || effectiveConfig.quality,
                          size: savedImageMetadata.size || effectiveConfig.size,
                          background: savedImageMetadata.background ?? effectiveConfig.background,
                          count: "1",
                      }
                    : { ...buildGenerationConfig(effectiveConfig, isAudioResult ? { ...sourceNode, metadata: { ...sourceNode.metadata, ...node.metadata } } : sourceNode, isAudioResult ? "audio" : node.type === CanvasNodeType.Text ? "text" : node.type === CanvasNodeType.Video ? "video" : "image"), count: "1" };
            if (!isAiConfigReady(generationConfig, generationConfig.model)) {
                openConfigDialog(true);
                return;
            }
            ...
            try {
                if (node.type === CanvasNodeType.Text && !isAudioResult) {
                    if (!context) return;
                    let streamed = "";
                    const answer = await requestImageQuestion(
                        generationConfig,
                ...
                if (isAudioResult) {
                    await completeAudioNodeTask(node.id, sourceNode.id, generationConfig, prompt, controller.signal);
                    return;
                }

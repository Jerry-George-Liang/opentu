...
import { NativeSettingsPanel } from "./native-settings-panel";
...
type VideoSettingsPanelProps = {
    config: AiConfig;
    onConfigChange: (key: "vquality" | "size" | "videoSeconds" | "videoGenerateAudio" | "videoWatermark" | "videoMode" | "nativeParams", value: string) => void;
    theme: CanvasTheme;
    showTitle?: boolean;
    className?: string;
};
...
export function VideoSettingsPanel({ config, onConfigChange, theme, showTitle = true, className = "w-[320px] space-y-4 rounded-2xl px-1 py-0.5" }: VideoSettingsPanelProps) {
    ...
    if (nativeModel(config, "video")) return <NativeSettingsPanel config={config} capability="video" onChange={(value) => onConfigChange("nativeParams", value)} theme={theme} showTitle={showTitle} className={className} />;

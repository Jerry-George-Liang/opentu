...
type ImageSettingsPanelProps = {
    config: AiConfig;
    onConfigChange: (key: "quality" | "size" | "count" | "background" | "nativeParams", value: string) => void;
    theme: CanvasTheme;
    showTitle?: boolean;
    className?: string;
    maxCount?: number;
...
export function ImageSettingsPanel({ config, onConfigChange, theme, showTitle = true, className = "w-[320px] space-y-4 rounded-2xl px-1 py-0.5", maxCount = 15, quickCount = 10 }: ImageSettingsPanelProps) {
    ...
    if (nativeModel(config, "image")) return <NativeSettingsPanel config={config} capability="image" onChange={(value) => onConfigChange("nativeParams", value)} theme={theme} showTitle={showTitle} className={className} />;

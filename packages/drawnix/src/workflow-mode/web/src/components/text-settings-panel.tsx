...
export function TextSettingsPanel({ config, onConfigChange, theme, className = "space-y-4" }: TextSettingsPanelProps) {
    ...
    if (nativeModel(config, "text")) return <NativeSettingsPanel config={config} capability="text" onChange={(value) => onConfigChange("nativeParams", value)} theme={theme} className={className} />;
    return (

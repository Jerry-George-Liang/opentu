...
export function CanvasNodePromptPanel({ node, nodes, isRunning, onPromptChange, onConfigChange, onGenerate, onStop, mentionReferences = [], connectedNodes = [], onDisconnectReference, onStartReferenceSelection, onImageSettingsOpenChange, modeOverride }: CanvasNodePromptPanelProps) {
    ...
    const mode = modeOverride ?? node.metadata?.generationMode ?? defaultMode(node.type);
                ...
                <div className="flex min-w-0 items-center gap-2">
                    ...
                    {mode === "image" ? (
                        ...
                        <>
                            ...
                            <CanvasTextSettingsPopover config={config} count={node.metadata?.textCount || 1} onConfigChange={(key, value) => onConfigChange(node.id, { [key]: value })} onCountChange={(textCount) => onConfigChange(node.id, { textCount })} />
                        </>
                    )}
                </div>
                <Button

...
type AudioSettingKey = "audioVoice" | "audioFormat" | "audioSpeed" | "audioInstructions" | "nativeParams";
...
export function AudioSettingsPanel({ config, onConfigChange, theme, showTitle = true, className = "w-[320px] space-y-4 rounded-2xl px-1 py-0.5" }: AudioSettingsPanelProps) {
    ...
    if (nativeModel(config, "audio")) return <NativeSettingsPanel config={config} capability="audio" onChange={(value) => onConfigChange("nativeParams", value)} theme={theme} showTitle={showTitle} className={className} />;
